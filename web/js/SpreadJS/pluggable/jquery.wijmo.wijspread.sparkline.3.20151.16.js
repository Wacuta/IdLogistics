...
(function(wijmo)
{
    (function(spread)
    {
        spread.feature("sparkline", ["core.common", "core.theme", "core.sheet_model"]);
        ...
        var Color = {FromArgb: function(a, r, g, b)
                {
                    return new spread._Color(a, r, g, b).toString()
                }};
        ...
        function normalizeColor(color, owner)
        {
            if (owner && color)
            {
                var c = spread._ThemeContext.getColor(owner, color);
                if (c)
                {
                    return c
                }
        ...
        })(spread.EmptyValueStyle || (spread.EmptyValueStyle = {}));
        var EmptyValueStyle = spread.EmptyValueStyle;
        ...
        })(spread.SparklineAxisMinMax || (spread.SparklineAxisMinMax = {}));
        var SparklineAxisMinMax = spread.SparklineAxisMinMax;
        ...
        spread.SparklineSetting = SparklineSetting;
        ...
        })(spread.SparklineType || (spread.SparklineType = {}));
        var SparklineType = spread.SparklineType;
        ...
        })(spread.DataOrientation || (spread.DataOrientation = {}));
        var DataOrientation = spread.DataOrientation;
        var SparklineGroup = (function()
            {
                ...
                {
                    var self = this;
                    spread.ArrayHelper.remove(self._innerList, item);
                ...
                {
                    return spread.ArrayHelper.contains(this._innerList, item)
                };
                ...
                SparklineGroup.prototype._getMaxMinValues = function(sparkline)
                {
                    ...
                    if (data)
                    {
                        ...
                        {
                            var val = values[i];
                            if (val === spread.__invalidValuePlaceHolder)
                            {
                                val = 0
                            }
        ...
        spread.SparklineGroup = SparklineGroup;
        ...
        spread._PositionRect = _PositionRect;
        var SparklineRender = (function()
            {
                ...
                SparklineRender.prototype._getCachedIndexMaping = function(cachedValues, cachedDatetimes)
                {
                    ...
                    var invalidValuePlaceHolder = spread.__invalidValuePlaceHolder,
                    ...
                    if (this.sparklineInfo().displayDateAxis())
                    {
                        ...
                        for (i = 0; i < sortedCount; i++)
                        {
                            ...
                            valueIndex = spread.ArrayHelper.indexOf(cachedDatetimes, datetime);
                            while (spread.ArrayHelper.contains(cachedIndexMapping, valueIndex))
                            {
                                valueIndex = spread.ArrayHelper.indexOf(cachedDatetimes, datetime, valueIndex + 1)
                            }
                            if (!isNaN(datetime))
                            {
                ...
                SparklineRender.prototype._getValue = function(valueIndex, cachedValues)
                {
                    ...
                    else if (item === spread.__invalidValuePlaceHolder)
                    {
                        item = 0
                    }
                    return item
                ...
                SparklineRender.prototype._getDataPointColor = function(indexInValueCache, cachedValues, cachedDatetimes)
                {
                    ...
                    if (typeof(value) !== const_undefined && value !== keyword_null)
                    {
                        ...
                        if (typeof(ret) === const_undefined || ret === keyword_null)
                        {
                            ...
                            {
                                var dateIndex1 = spread.ArrayHelper.indexOf(cachedIndexMaping, indexInValueCache);
                        ...
                        if (typeof(ret) === const_undefined || ret === keyword_null)
                        {
                            if (group.displayDateAxis)
                            {
                                var dateIndex2 = spread.ArrayHelper.indexOf(cachedIndexMaping, indexInValueCache);
                ...
                SparklineRender.prototype._getItemX = function(availableSize, index, cachedValues, cachedDatetimes, zoomFactor)
                {
                    ...
                    {
                        itemWidth = self._getItemWidth(availableSize, cachedValues, cachedDatetimes, zoomFactor);
                        var valueIndex = spread.ArrayHelper.indexOf(self._getCachedIndexMaping(cachedValues, cachedDatetimes), index),
        ...
        spread.SparklineRender = SparklineRender;
        ...
        var Sparkline = (function()
            {
                ...
                Sparkline.prototype.fromJSON = function(jsonData, isNoneSchema)
                {
                    ...
                    if (jsonData.data)
                    {
                        ...
                        self._data = new spread.Range(cr.row, cr.col, cr.rowCount, cr.colCount)
                    }
                };
                return Sparkline
            })();
        spread.Sparkline = Sparkline;
        var WorksheetSparklineGroupManager = (function()
            {
                ...
                {
                    spread.ArrayHelper.remove(this.groups(), group)
                };
                WorksheetSparklineGroupManager.prototype.contains = function(group)
                {
                    return spread.ArrayHelper.contains(this.groups(), group)
                };
                ...
                {
                    var self = this;
                    var moveRange = new spread.Range(fromRow, fromColumn, rowCount, columnCount);
                    var offsetRow = toRow - fromRow;
                    fromRow;
                    var offsetColumn = toColumn - fromColumn;
                    for (var i = 0; i < self._groups.length; i++)
                ...
                WorksheetSparklineGroupManager.prototype._addRowRange = function(row, rowCount, range)
                {
                    if (range)
                    {
                        ...
                        {
                            return new spread.Range(range.row, range.col, range.rowCount + rowCount, range.colCount)
                        }
                        else
                        {
                            return new spread.Range(range.row + rowCount, range.col, range.rowCount, range.colCount)
                        }
                    }
                    return keyword_null
                };
                WorksheetSparklineGroupManager.prototype._addColumnRange = function(column, columnCount, range)
                {
                    if (range)
                    {
                        ...
                        {
                            return new spread.Range(range.row, range.col, range.rowCount, range.colCount + columnCount)
                        }
                        else
                        {
                            return new spread.Range(range.row, range.col + columnCount, range.rowCount, range.colCount)
                        }
                    }
                    return keyword_null
                };
                WorksheetSparklineGroupManager.prototype._removeColumnRange = function(column, columnCount, range)
                {
                    ...
                    return new spread.Range(range.row, ret.start, range.rowCount, ret.end - ret.start + 1)
                };
                WorksheetSparklineGroupManager.prototype._removeRowRange = function(row, rowCount, range)
                {
                    ...
                    return new spread.Range(ret.start, range.col, ret.end - ret.start + 1, range.colCount)
                };
        ...
        spread.WorksheetSparklineGroupManager = WorksheetSparklineGroupManager;
        ...
        spread.SparklineValue = SparklineValue
