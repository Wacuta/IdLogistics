...
(function(wijmo)
{
    (function(_spread)
    {
        _spread.feature("touch", ["core.common", "core.sheet_action", "core.sheet_model", "core.sheet_border"]);
        ...
        var TouchEventProvider = (function()
            {
                ...
                TouchEventProvider.prototype.attachDettach = function(targetElement, isAttach)
                {
                    ...
                    var element = targetElement.element,
                        elements = self._elements;
                    var elementIndex = _spread.ArrayHelper.indexOf(elements, targetElement);
        ...
        _spread.TouchEventProvider = TouchEventProvider;
        ...
        _spread.TouchHelper = TouchHelper;
        ...
        var TouchZoomManager = (function()
            {
                ...
                TouchZoomManager.prototype._getTouchCanvas = function(width, height)
                {
                    ...
                    if (!canvas)
                    {
                        ...
                        _spread.DPIHelper.adjustDevicePixel(canvas, null, this._sheet);
                        _spread.DPIHelper.setSize(canvas, width, height)
                    }
                    else if (width !== _spread.DPIHelper.getLogicWidth(canvas) || height !== _spread.DPIHelper.getLogicHeight(canvas))
                    {
                        _spread.DPIHelper.setSize(canvas, width, height)
                    }
                    return canvas
                ...
                {
                    var self = this;
                    var canvasWidth = _spread.DPIHelper.getLogicWidth(srcCanvas),
                        canvasHeight = _spread.DPIHelper.getLogicHeight(srcCanvas);
                    var canvas = self._getTouchCanvas(canvasWidth, canvasHeight);
                    var ctx = canvas.getContext("2d"),
                        ratioX = _spread.DPIHelper.getScaleX(canvas),
                        ratioY = _spread.DPIHelper.getScaleY(canvas);
                ...
                TouchZoomManager.prototype._restoreCanvas = function(destCanvas, newZoomFactor, grayAreaBackColor)
                {
                    var self = this,
                        ...
                        destW,
                        destH,
                        canvasWidth = _spread.DPIHelper.getLogicWidth(destCanvas),
                        canvasHeight = _spread.DPIHelper.getLogicHeight(destCanvas),
                    ...
                    var ctx = destCanvas.getContext("2d"),
                        ratioX = _spread.DPIHelper.getScaleX(destCanvas),
                        ratioY = _spread.DPIHelper.getScaleY(destCanvas);
        ...
        var TouchScrollManager = (function()
            {
                ...
                {
                    var colLayouts = new _spread._LayoutModel;
                    ...
                    for (; viewportWidth > 0 && col < colCount; col++)
                    {
                        ...
                        colLayouts.push(new _spread._Layout(-1, col, colX, -1, colWidth, -1));
                        colX += colWidth;
                ...
                {
                    var rowLayouts = new _spread._LayoutModel;
                    var sheet = this.sheet,
                        cachePool = sheet._cachePool,
                        layout = sheet._getSheetLayout(),
                    ...
                    for (; viewportHeight > 0 && row < rowCount; row++)
                    {
                        ...
                        rowLayouts.push(new _spread._Layout(row, -1, -1, rowY, -1, rowHeight));
                        rowY += rowHeight;
                ...
                TouchScrollManager.prototype._vScrollTo = function(translationY, isInertia, bufferRect, clipRect)
                {
                    ...
                    if (_spread.util._useDoubleBuffer())
                    {
                ...
                {
                    var useWijmoTheme = this.sheet.parent.useWijmoTheme,
                        fillStyle;
                    if (useWijmoTheme)
                    {
                        var themeStyle = _spread.Global.prototype.getWijmoThemeStyle(0);
                        fillStyle = themeStyle && themeStyle.backgroundColor
                    }
                    else
                    {
                ...
                TouchScrollManager.prototype._continueScrollCore = function(deltaTranslationX, deltaTranslationY, dir, isInertia)
                {
                    ...
                    if (paintH && paintV)
                    {
                        render.paintBody(ctx, new _spread.Rect(clipRect.x, clipRect.y, clipRect.width, clipRect.height), new _spread.Rect(clipRect2.x, clipRect2.y, clipRect2.width, clipRect2.height))
                    }
                    else
                    {
                        render.paintBody(ctx, new _spread.Rect(clipRect.x, clipRect.y, clipRect.width, clipRect.height))
                    }
                    self._paintAdornment(ctx, new _spread.Rect(x, y, width, height));
                        ...
                        else
                        {
                            ...
                            if (cellTypeHitInfo)
                            {
                                ...
                                if ((sheetArea === keyword_null || sheetArea === keyword_undefined || sheetArea === 3) && (row !== oldActiveRow || col !== oldActiveCol))
                                {
                                    ...
                                    if (span)
                                    {
                                        newSels = [new _spread.Range(span.row, span.col, span.rowCount, span.colCount)]
                                    }
                                    else
                                    {
                                        newSels = [new _spread.Range(row, col, 1, 1)]
                                    }
                    ...
                    if (currentTarget)
                    {
                        ...
                        if (resizeInfo)
                        {
                            if (resizeInfo.action === "sizeRow")
                            {
                                ...
                                action = new _spread.UndoRedo.RowAutoFitUndoAction(sheet, rowList, resizeInfo.sheetArea === 1);
                            ...
                            else
                            {
                                ...
                                action = new _spread.UndoRedo.ColumnAutoFitUndoAction(sheet, columnList, resizeInfo.sheetArea === 2);
                                sheet._doCommand(action)
                            }
                            return
                        }
        ...
        _spread.TouchManager = TouchManager;
        ...
        var TouchToolStrip = (function(_super)
            {
                ...
                {
                    _super.call(this, _spread.util.getPreferredZIndex(spread._host));
                    var self = this;
                    self._useWijmoTheme = false;
                    ...
                    self._defaultSeprateLineColor = "#CCC";
                    self._domDiv = "div";
                ...
                TouchToolStrip.prototype.open = function(x, y)
                {
                    ...
                    _spread.FocusHelper.setActiveElement(keyword_null);
                ...
                TouchToolStrip.prototype.useWijmoTheme = function(value)
                {
                    var self = this;
                    ...
                    if (typeof value === 'boolean' && self._useWijmoTheme !== value)
                    {
                        ...
                        var toolStripDialog = self._toolStripDialog,
                            text = toolStripDialog.find("span." + self._textClass),
                            toolstripButton = toolStripDialog.find("button." + self._buttonClass),
                            separatorLine = toolStripDialog.find("div." + self._seprateLineClass),
                            toolStripDialogWijmoClass = "ui-state-default",
                            toolStripDialogNotWijmoClass = "toolstrip-default",
                            toolstripButtonWijmoClass = "ui-state-default ui-widget",
                            toolstripButtonNotWijmoClass = self._buttonStyleClass;
                        toolStripDialog.removeClass(value ? toolStripDialogNotWijmoClass : toolStripDialogWijmoClass);
                        toolStripDialog.addClass(value ? toolStripDialogWijmoClass : toolStripDialogNotWijmoClass);
                        toolstripButton.addClass(value ? toolstripButtonWijmoClass : toolstripButtonNotWijmoClass);
                        toolstripButton.removeClass(value ? toolstripButtonNotWijmoClass : toolstripButtonWijmoClass);
                        separatorLine.css("background-color", value ? text.css("color") : self._defaultSeprateLineColor);
                        return self
                    }
                    return keyword_null
                };
                TouchToolStrip.prototype.repaint = function()
                {
                    this.useWijmoTheme(this.spreadHost.useWijmoTheme)
                };
                ...
                TouchToolStrip.prototype._initDialog = function()
                {
                    ...
                    if (!self._toolStripDialog)
                    {
                        (self._toolStripDialog = self.getContainer()).addClass("toolstrip-default").appendTo(document.body).hide();
                ...
                TouchToolStrip.prototype._setDefaultCellDomString = function()
                {
                    ...
                    self._pasteItem = new TouchToolStripItem(self._getItemName(self.pasteName), _spread.SR.ToolStrip_PasteText, pasteRes, self._doTapPaste);
                    self._cutItem = new TouchToolStripItem(self._getItemName(self.cutName), _spread.SR.ToolStrip_CutText, cutRes, self._doTapCut);
                    self._copyItem = new TouchToolStripItem(self._getItemName(self.copyName), _spread.SR.ToolStrip_CopyText, copyRes, self._doTapCopy);
                    self._autoFillItem = new TouchToolStripItem(self._getItemName(self.autoFillName), _spread.SR.ToolStrip_AutoFillText, autoFillRes, self._showAutoFillIndicator, self._hideAutoFill);
                ...
                TouchToolStrip.prototype._getMenuItemString = function(item, name)
                {
                    ...
                    if (imgSpan && name)
                    {
                        ...
                        {
                            buttonStyleClass = "ui-state-default ui-widget"
                        }
                        else
                        {
                            buttonStyleClass = self._buttonStyleClass
                        }
                        var buttonDom = self._getDomElementString(self._domButton, name, imgSpan + textSpan, {
                                "class": self._buttonClass + " " + buttonStyleClass, style: "border: 0px;" + "padding:4px;" + "margin:3px;" + "min-width:" + self._itemMinWidth + "px;"
                            });
                        return buttonDom
                    }
                    else
                ...
                TouchToolStrip.prototype._updateResource = function()
                {
                    ...
                    $("#" + self._pasteItem._name).find("span." + textClass).text(_spread.SR.ToolStrip_PasteText);
                    $("#" + self._cutItem._name).find("span." + textClass).text(_spread.SR.ToolStrip_CutText);
                    $("#" + self._copyItem._name).find("span." + textClass).text(_spread.SR.ToolStrip_CopyText);
                    $("#" + self._autoFillItem._name).find("span." + textClass).text(_spread.SR.ToolStrip_AutoFillText)
                };
        ...
        _spread.TouchToolStrip = TouchToolStrip;
        ...
        _spread.TouchToolStripItem = TouchToolStripItem;
        ...
        _spread.TouchToolStripSeparator = TouchToolStripSeparator;
        ...
        _spread.TouchMouseMessageFilter = TouchMouseMessageFilter;
        var EventMode;
        (function(EventMode)
        {
        ...
        var TabStripTouchEventHandler = (function()
            {
                ...
                TabStripTouchEventHandler.prototype.doManipulationStarted = function(e)
                {
                    ...
                    else if (hitTestInfo.element === hit_element_tab || hitTestInfo.element === hit_element_newTab || hitTestInfo.element === "")
                    {
                        var srcCanvas = tab.canvas,
                            canvasWidth = _spread.DPIHelper.getLogicWidth(srcCanvas),
                            canvasHeight = _spread.DPIHelper.getLogicHeight(srcCanvas);
                        var canvas = self._getTouchCanvas(canvasWidth, canvasHeight);
                        var ctx = canvas.getContext("2d"),
                            ratioX = _spread.DPIHelper.getScaleX(canvas),
                    ...
                    else
                    {
                        ...
                        if (translationX !== 0)
                        {
                            var destCanvas = tab.canvas,
                                ...
                                ratioY = _spread.DPIHelper.getScaleY(destCanvas);
                            ctx.save();
                            ctx.clearRect(0, 0, _spread.DPIHelper.getLogicWidth(destCanvas), _spread.DPIHelper.getLogicHeight(destCanvas));
                            ...
                            if (translationX > 0)
                            {
                                ...
                                if (width > 0)
                                {
                                    ...
                                    tab.paintTabs(ctx, new _spread.Rect(x, y, width, height));
                                    ctx.translate(-xOffset, 0)
                                }
                            }
                            else if (translationX < 0)
                ...
                TabStripTouchEventHandler.prototype._getTouchCanvas = function(width, height)
                {
                    ...
                    {
                        canvas = document.createElement("canvas");
                        _spread.DPIHelper.adjustDevicePixel(canvas, this._tab._spread);
                        _spread.DPIHelper.setSize(canvas, width, height)
                    }
                    else if (width !== _spread.DPIHelper.getLogicWidth(canvas) || height !== _spread.DPIHelper.getLogicHeight(canvas))
                    {
        ...
        _spread.TabStripTouchManager = TabStripTouchManager;
        ...
        _spread.FloatingObjectTouchManager = FloatingObjectTouchManager;
                ...
                FloatingObjectTouchEventHandler.prototype.doTapped = function(e)
                {
                    try
                    {
                        ...
                        if (!this._floatingObject.isSelected())
                        {
                            ...
                            _spread.FocusHelper.setActiveElement(sheet)
                        }
        ...
        _spread.TouchTargetElement = TouchTargetElement;
        var _SimulateMouseEvents = (function()
            {
                ...
                _SimulateMouseEvents.prototype.simulateMouseEvent = function(event, simulatedType)
                {
                    ...
                    _spread.util.cancelDefault(event);
        ...
        _spread._SimulateMouseEvents = _SimulateMouseEvents;
        ...
        _spread.CommentTouchManager = CommentTouchManager;
        ...
        _spread.CommentContentTouchManager = CommentContentTouchManager;
