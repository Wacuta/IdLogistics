...
(function(wijmo)
{
    (function(spread)
    {
        spread.feature("fill", ["core.common", "core.globalize", "core.stringResource"]);
        ...
        })(spread.FillType || (spread.FillType = {}));
        var FillType = spread.FillType;
        ...
        })(spread.FillDateUnit || (spread.FillDateUnit = {}));
        var FillDateUnit = spread.FillDateUnit;
        function __createRange(row, column, rowCount, columnCount)
        {
            return new spread.Range(row, column, rowCount, columnCount)
        }
        function __createDateTimeHelper(date)
        {
            return new spread._DateTimeHelper(date)
        }
        function __isNumber(value)
        {
            ...
            return spread.FormatConverter.IsNumber(value)
        }
        function __fromOADate(value)
        {
            return spread._DateTimeHelper.fromOADate(value)
        }
        ...
        var NumberSource = (function()
            {
                ...
                NumberSource.prototype.indexes2 = function()
                {
                    ...
                    if (count > 0 && spread.util.hasCalc())
                    {
                        ...
                        return new spread.Calc._ConcreteArray(value, count)
                    }
                    return keyword_null
                };
                ...
                NumberSource.prototype.insert = function(insertIndex, index, value)
                {
                    if (!__isNumber(value))
                    {
                        throw new Error(spread.SR.Exp_NumberOnly);
                    }
                    var self = this;
                    ...
                    spread.ArrayHelper.insert(self._indexes, insertIndex, index);
                    spread.ArrayHelper.insert(self._innerValues, insertIndex, self.toDouble(value))
                };
                ...
                {
                    return spread.FormatConverter.ToDouble(value)
                };
                NumberSource.prototype.toDateTime = function(value)
                {
                    ...
                    else if (spread.util.hasCalc())
                    {
                        try
                        {
                            ret = spread.Calc.Convert.DT(value)
                        }
                        catch(err)
                        {
                            ret = value
                    ...
                    return (function evalateStandardFunction(evaluator, values, indexes, dest)
                        {
                            ...
                            var fn = spread.Calc.Functions.findGlobalFunction(name);
        ...
        var FillImp = (function()
            {
                ...
                FillImp.prototype.seriesTrendFillRange = function(range, fillSeries, fillType, justGetTooltipContent)
                {
                    ...
                    {
                        throw new Error(spread.SR.Exp_RangeContainsMergedCell);
                    }
                    var sourceData,
                        targetData,
                        i;
                ...
                {
                    if (sourceData && sourceData.dataCount() > 0 && spread.util.hasCalc())
                    {
                        if (type === 1 || type === 2)
                        {
                            var newValues = [];
                ...
                FillImp.prototype.fillAuto = function(range, series, justGetToolTipContent)
                {
                    ...
                    if (sourceRange)
                    {
                        ...
                        {
                            throw new Error(spread.SR.Exp_ChangeMergedCell);
                        }
                        var isIncrease = true;
                ...
                FillImp.prototype.autoFillRowTrendValues = function(sourceRange, targetRange, sourceRow, targetRow, trendData, justGetTooltipContent, isIncrease, colOffset)
                {
                    ...
                    var count,
                        ...
                        sourceColumn,
                        targetColumn;
                    if (trendData && trendData.dataCount() > 0 && spread.util.hasCalc())
                    {
                        ...
                        if (self.isArithmeticProgression(trendData._indexes, trendData._innerValues))
                        {
                            ...
                            var trendIndexes = new spread.Calc._ConcreteArray(value, count);
                            for (i = 0; i < count; i++)
                            {
                ...
                FillImp.prototype.copyCell = function(sheet, fromRow, fromColumn, toRow, toColumn, newValue, type)
                {
                    ...
                    {
                        var clonedStyle = new spread.Style;
                ...
                {
                    var self = this;
                    var _fillRangeHasMergedCellErrorMessage = spread.SR.Exp_TargetContainsMergedCells;
                    var _fillRangeHaveDifferentSizeErrorMessage = spread.SR.Exp_MergedCellsIdentical;
                    var nWhole,
                        nPartial,
                        targetRange,
                        i;
...
(function(wijmo)
{
    (function(spread)
    {
        spread.feature("fill_ui", ["core.common", "core.sheet_action", "core.stringResource"]);
        (function(_FillSmartTag)
        {
            _FillSmartTag[_FillSmartTag["Tag"] = 0] = "Tag";
            _FillSmartTag[_FillSmartTag["Down"] = 1] = "Down"
        })(spread._FillSmartTag || (spread._FillSmartTag = {}));
        var _FillSmartTag = spread._FillSmartTag;
        var _GcFillDialog = (function(_super)
            {
                ...
                {
                    var self = this;
                    self.tagContainer = self.getContainer().addClass("container-default-header tag-container no-user-select");
                ...
                _GcFillDialog.prototype.useWijmoTheme = function(value)
                {
                    var self = this;
                    if (typeof value === "undefined")
                    {
                    ...
                    if (typeof value === 'boolean' && self._useWijmoTheme !== value)
                    {
                        ...
                        var tagContainer = self.tagContainer,
                            wijmoClassName = "ui-widget-header",
                            notWijmoClassName = "container-default-header";
                        tagContainer.removeClass(value ? notWijmoClassName : wijmoClassName);
                        tagContainer.addClass(value ? wijmoClassName : notWijmoClassName);
                        return self
                    }
                    return null
                };
                ...
                {
                    var self = this,
                        device = spread.util.device(),
                        isSafariOnIpad = $.browser.safari && (device.ipad || device.iphone);
                    self.tagContainer.hover(function()
                    {
                        var hoverClassName = self._useWijmoTheme ? "ui-state-hover" : "container-default-hover",
                    ...
                    {
                        var toggleClassName = self._useWijmoTheme ? "ui-state-active" : "container-default-active";
                        if (isSafariOnIpad)
                        {
                            self.smartTagDown.show()
                        }
                ...
                _GcFillDialog.prototype.open = function()
                {
                    ...
                    if (self.tagContainer)
                    {
                        ...
                        var tempSpread = sheet.parent,
                            useTheme = (tempSpread && typeof(tempSpread.useWijmoTheme) != "undefined") ? tempSpread.useWijmoTheme : false;
                        self.useWijmoTheme(useTheme)
                    }
                };
                ...
                _GcFillDialog.prototype._openMenu = function()
                {
                    ...
                    if (!self.menuDialog)
                    {
                        ...
                        self.menuDialog.useWijmoTheme(self._useWijmoTheme)
                    }
        ...
        spread._GcFillDialog = _GcFillDialog;
        var _GcFillMenuDialog = (function(_super)
            {
                ...
                _GcFillMenuDialog.prototype._initMenuDialog = function()
                {
                    ...
                    self.copyCells = self._createMenuItem("smartMenuCopyCells", self.itemClass, spread.SR.CopyCells, "0", true).appendTo(self.menuContainer);
                    self.fillSeries = self._createMenuItem("smartMenuFillSeries", self.itemClass, spread.SR.FillSeries, "1").appendTo(self.menuContainer);
                    self.fillFormattingOnly = self._createMenuItem("smartMenuFillFormattingOnly", self.itemClass, spread.SR.FillFormattingOnly, "2").appendTo(self.menuContainer);
                    self.fillWithoutFormatting = self._createMenuItem("smartMenuFillWithoutFormatting", self.itemClass, spread.SR.FillWithoutFormatting, "3").appendTo(self.menuContainer)
                };
                _GcFillMenuDialog.prototype._createMenuItem = function(id, itemClass, text, inputValue, checked)
                {
                    ...
                    var outerDiv = $("<div>").css({
                        ...
                        }).addClass("container-default-state " + itemClass);
                ...
                _GcFillMenuDialog.prototype.useWijmoTheme = function(value)
                {
                    var self = this;
                    if (typeof value === 'undefined')
                    {
                    ...
                    {
                        _super.prototype.useWijmoTheme.call(this, value);
                        var itemOuterDiv = self.menuContainer.find("div" + '.' + self.itemClass),
                            checkedItemOuterDiv = itemOuterDiv.find("input:checked").parent(),
                            itemOuterDivWijmoClass = "ui-state-default",
                            itemOuterDivNotWijmoClass = "container-default-state",
                            checkedItemWijmoClass = "ui-icon ui-icon-check",
                            checkedItemNotWijmoClass = "check-image";
                        itemOuterDiv.removeClass(value ? itemOuterDivNotWijmoClass : itemOuterDivWijmoClass);
                        itemOuterDiv.addClass(value ? itemOuterDivWijmoClass : itemOuterDivNotWijmoClass);
                        checkedItemOuterDiv.removeClass(value ? checkedItemNotWijmoClass : checkedItemWijmoClass);
                        checkedItemOuterDiv.addClass(value ? checkedItemWijmoClass : checkedItemNotWijmoClass);
                        return self
                    }
                    return null
                };
                _GcFillMenuDialog.prototype._attachEvent = function()
                {
                    ...
                    {
                        var className = self._useWijmoTheme ? "ui-state-hover" : "container-default-hover";
                    ...
                    {
                        var smartMenuRadioClassName = self._useWijmoTheme ? "ui-icon ui-icon-check" : "check-image";
                ...
                _GcFillMenuDialog.prototype._changeFill = function(fillType)
                {
                    ...
                    var fillUndoAction = new spread.UndoRedo.FillUndoAction(sheet, fillType, true, sheet._eventHandler._currentFillRange);
