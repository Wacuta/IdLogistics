...
(function(wijmo)
{
    (function(spread)
    {
        spread.feature("group", ["core.common", "core.stringResource"]);
        ...
        })(spread.GroupState || (spread.GroupState = {}));
        var GroupState = spread.GroupState;
        ...
        spread.RangeGroupInfo = RangeGroupInfo;
        ...
        spread.WorkingState = WorkingState;
        var RangeGroup = (function()
            {
                ...
                RangeGroup.prototype.group = function(index, count)
                {
                    ...
                    {
                        throw new Error(spread.SR.Exp_InvalidIndex);
                    }
                    if (!self._isIndexValid(index + count - 1))
                    {
                        throw new Error(spread.SR.Exp_InvalidCount);
                    }
                    self._rootCached = keyword_null;
                ...
                RangeGroup.prototype._expand = function(index, level, expand)
                {
                    ...
                    if (level < -1)
                    {
                        throw new Error(spread.SR.Exp_InvalidLevel);
                    }
                ...
                RangeGroup.prototype.expandGroup = function(groupInfo, expand)
                {
                    if (!groupInfo)
                    {
                        throw new Error(spread.SR.Exp_GroupInfoIsNull);
                    }
                    var summaryItemIndex = -1;
                ...
                RangeGroup.prototype._move = function(from, to, count)
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        var copiedItems = [];
                        var index = spread.ArrayHelper.nextNonEmptyIndex(self.items, from - 1);
                        while (index >= 0 && index < (from + count))
                        {
                            ...
                            index = spread.ArrayHelper.nextNonEmptyIndex(self.items, index)
                        }
                        spread.ArrayHelper.clear(self.items, from, count);
                        spread.ArrayHelper.clear(self.items, to, count);
        ...
        spread.RangeGroupItemInfo = RangeGroupItemInfo;
        ...
        spread.GroupedItemIndexEnumerator = GroupedItemIndexEnumerator;
        var ThemeHelper = {
                getThemeBackgroundColor: function(sheet, defaultColor)
                {
                    if (sheet.parent && sheet.parent.useWijmoTheme)
                    {
                        var themeStyle = spread.Global.prototype.getWijmoThemeStyle(0);
                            ...
                            if (groupChildrenCount > 0)
                            {
                                for (i = 0; i < groupChildrenCount; i++)
                                {
                                    ...
                                    if (childGroup.getState() === 1)
                                    {
                                        ...
                                        {
                                            spread.ArrayHelper.remove(dots, j)
                                        }
                                    }
                                    if (groupDirection === 1)
                                    {
                                        spread.ArrayHelper.remove(dots, childGroupEnd + 1)
                                    }
                                    else if (groupDirection === 0)
                                    {
                                        spread.ArrayHelper.remove(dots, childGroupStart - 1)
                                    }
                                ...
                                for (var ci = 0; ci < groupChildrenCount; ci++)
                                {
                                    ...
                                    if (child)
                                    {
                                        ...
                                        else if ((groupDirection === 1 && childEnd === groupEndIndex) || (groupDirection === 0 && childStart === groupStartIndex))
                                        {
                                            ...
                                            {
                                                spread.ArrayHelper.remove(dots, ic)
                                            }
                                        }
                                    }
                                }
                    ...
                    if (groupDotInfosCount > 0)
                    {
                        for (i = 0; i < groupDotInfosCount; i++)
                        {
                            ...
                            if (rowLayout && rowLayout.height >= LINE_SIZE)
                            {
                                ...
                                ctx.fillStyle = ThemeHelper.getThemeForeColor(sheet, "black");
                ...
                _RangeGroupPresenter.prototype.getRowGroupButton = function(x, y)
                {
                    ...
                    if (groupButtonInfosCount > 0)
                    {
                        for (var i = 0; i < groupButtonInfosCount; i++)
                        {
                            ...
                            var btnRect = new spread.Rect(groupLayout.x + columnX + 0.5, groupLayout.y + rowY + 0.5, rectWidth, rectHeight);
        ...
        spread._RangeGroupPresenter = _RangeGroupPresenter;
        var _RangeGroupHeaderPresenter = (function()
            {
                ...
                _RangeGroupHeaderPresenter.prototype.paintGroupHeader = function(ctx)
                {
                    ...
                    ctx.fillStyle = ThemeHelper.getThemeBackgroundColor(sheet, "#D7E6F7");
                    ctx.strokeStyle = "gray";
                ...
                _RangeGroupHeaderPresenter.prototype.getGroupButton = function(x, y)
                {
                    ...
                    if (rowGroup)
                    {
                        if (sheet.colHeaderVisible && layout.colHeaderHeight >= minSize)
                        {
                            ...
                            for (i = 0; i < buttonCount; i++)
                            {
                                btnRect = new spread.Rect(groupLayout.x + columnX, groupLayout.y + rowY, minSize, minSize);
                                if (btnRect.contains(x, y))
                                {
                                    return {index: i}
                                }
        ...
        spread._RangeGroupHeaderPresenter = _RangeGroupHeaderPresenter
