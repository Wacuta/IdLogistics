...
(function(wijmo)
{
    (function(spread)
    {
        spread.feature("basedialog", ["core.common"]);
        var BaseDialog = (function()
            {
                ...
                BaseDialog.prototype._init = function()
                {
                    ...
                    if (!self._hasTargetContainer(self._name))
                    {
                        ...
                        self.container = $("<div></div>").addClass(self._wijpopupClass).attr({
                ...
                BaseDialog.prototype.useWijmoTheme = function(value)
                {
                    var self = this;
                    if (typeof value === "undefined")
                    {
                        return self._useWijmoTheme
                    }
                    if (typeof value === 'boolean' && self._useWijmoTheme !== value)
                    {
                        self._useWijmoTheme = value;
                        if (value)
                        {
                            self.container.addClass("ui-widget ui-widget-content")
                        }
                        else
                        {
                            self.container.removeClass("ui-widget ui-widget-content")
                        }
                        return self
                    }
                    return null
...
(function(wijmo)
{
    (function(spread)
    {
        spread.feature("filter_ui", ["core.common", "core.stringResource", "core.spreadpanelex"]);
        var keyword_null = null,
            ...
            wijmoHoverClass = "ui-state-hover",
            notWijmoHoverClass = "container-default-hover",
        ...
        })(spread._FilterButtonState || (spread._FilterButtonState = {}));
        var _FilterButtonState = spread._FilterButtonState;
        ...
        spread._FilterButtonInfo = _FilterButtonInfo;
        ...
        spread._FilterButtonInfoModel = _FilterButtonInfoModel;
        var _GcFilterDialog = (function(_super)
            {
                __extends(_GcFilterDialog, _super);
                function _GcFilterDialog(sheet, filterButtonInfo)
                {
                    _super.call(this, spread.util.getPreferredZIndex(sheet && sheet.parent && sheet.parent._host));
                    this._allValues = [];
                ...
                _GcFilterDialog.prototype.open = function()
                {
                    ...
                    var useTouchLayout = (tempSpread && tempSpread.useTouchLayout) ? tempSpread.useTouchLayout() : false,
                        useWijmoTheme = (tempSpread && typeof(tempSpread.useWijmoTheme) != const_undefined) ? tempSpread.useWijmoTheme : false;
                    self._useTouchLayout = useTouchLayout;
                    self._initData(self.filterButtonInfo.col);
                    self._initLayout();
                    self._updateItemCache();
                    self._attachEvent();
                    self.useWijmoTheme(useWijmoTheme)
                };
                ...
                _GcFilterDialog.prototype.useWijmoTheme = function(value)
                {
                    var self = this;
                    if (typeof value === const_undefined)
                    {
                        return self._useWijmoTheme
                    }
                    if (typeof value === 'boolean' && self._useWijmoTheme !== value)
                    {
                        _super.prototype.useWijmoTheme.call(this, value);
                        if (self._scrollbarH)
                        {
                            self._scrollbarH.useWijmoTheme(value)
                        }
                        if (self._scrollbarV)
                        {
                            self._scrollbarV.useWijmoTheme(value)
                        }
                        var filterCheck = $("." + filterCheckClass);
                        var filterButton = $("." + filterButtonClass);
                        var filterSort = $("." + filterSortClass);
                        var filterItems = $("." + filterItemClass);
                        var checkAll = $("#" + checkAllID);
                        var uncheckAll = $("#" + unCheckAllID);
                        var sortContainer = $(".sort-container");
                        var layoutTable = $("." + layoutTableClass);
                        if (value)
                        {
                            self.container.removeClass(filterDialogStyleClass);
                            self.searchOuterDiv.removeClass("container-default-header");
                            filterButton.removeClass("container-default-state");
                            filterCheck.removeClass(filterCheckStyleClass);
                            self.searchOuterDiv.addClass("ui-widget-header ui-corner-all ui-multiselect-header ui-helper-clearfix");
                            filterSort.addClass("ui-corner-all");
                            filterItems.addClass("ui-corner-all");
                            filterButton.addClass("ui-button ui-state-default ui-corner-all");
                            checkAll.find("span:eq(0)").addClass("ui-icon ui-icon-check");
                            uncheckAll.find("span:eq(0)").addClass("ui-icon ui-icon-closethick");
                            layoutTable.addClass("ui-menu");
                            sortContainer.addClass("ui-menu-item")
                        }
                        else
                        {
                            self.searchOuterDiv.remove("ui-widget-header ui-corner-all ui-multiselect-header ui-helper-clearfix");
                            filterSort.removeClass("ui-corner-all");
                            filterItems.removeClass("ui-corner-all");
                            filterButton.removeClass("ui-button ui-state-default ui-corner-all");
                            checkAll.find("span:eq(0)").removeClass("ui-icon ui-icon-check");
                            uncheckAll.find("span:eq(0)").removeClass("ui-icon ui-icon-closethick");
                            layoutTable.removeClass("ui-menu");
                            sortContainer.removeClass("ui-menu-item");
                            self.container.addClass(filterDialogStyleClass);
                            filterCheck.addClass(filterCheckStyleClass);
                            self.searchOuterDiv.addClass("container-default-header");
                            filterButton.addClass("container-default-state")
                        }
                        return self
                    }
                    return keyword_null
                ...
                {
                    var self = this;
                    var ascLink = self._getSortLink(sortAscID, spread.SR.SortAscending).css("margin-top", "6px");
                    var desLink = self._getSortLink(sortDesID, spread.SR.SortDescending);
                    ...
                    self.searchOuterDiv = $("<div>").addClass("search-outer-div container-default-header");
                    var searchDiv = $("<div>").appendTo(self.searchOuterDiv);
                    var searchInput = $("<input>").attr({
                            type: "search", placeholder: spread.SR.Search, id: searchID
                        }).appendTo(searchDiv);
                    self.elementList.push(searchInput[0]);
                    var checkDiv = $("<div>").appendTo(self.searchOuterDiv).addClass("filter-check-outerDiv");
                    var checkAllLink = self._getCheckLink(checkAllID, spread.SR.CheckAll, "check-image");
                    var unCheckAllLink = self._getCheckLink(unCheckAllID, spread.SR.UncheckAll, "uncheck-image");
                    ...
                    var okButton = $("<button>").text(spread.SR.OK).addClass(filterButtonClass + " container-default-state").attr({id: okID});
                    var cancelButton = $("<button>").text(spread.SR.Cancel).addClass(filterButtonClass + " container-default-state").attr({id: cancelID});
                ...
                _GcFilterDialog.prototype._createTableLayout = function()
                {
                    ...
                    $(self.table).find("tr:eq(1) td:eq(1)").addClass("filter-sort-desc-container sort-container");
                    $(self.table).find("tr:eq(0) td:eq(1)").addClass("filter-sort-asc-container sort-container");
                    self.container.css({
                ...
                _GcFilterDialog.prototype._attachEvent = function()
                {
                    ...
                    $("#" + sortAscID).bind({
                        mouseup: function(event)
                        {
                            ...
                            self.close();
                            spread.util.cancelDefault(event)
                    ...
                    {
                        var hoverClassName = "filter-hover " + (self._useWijmoTheme ? wijmoHoverClass : notWijmoHoverClass);
                        var hoverItem = $(".filter-hover");
                        hoverItem.removeClass(wijmoHoverClass + " " + notWijmoHoverClass);
                    ...
                    $("." + filterButtonClass).hover(function()
                    {
                        ...
                        var filterButtonHoverClassName = "filter-button-hover " + (self._useWijmoTheme ? wijmoHoverClass : notWijmoHoverClass);
                        hoverItem.removeClass(wijmoHoverClass + " " + notWijmoHoverClass);
                ...
                _GcFilterDialog.prototype._filterKeyDown = function(event)
                {
                    ...
                    var hoverItem = keyword_null;
                    if (self._useWijmoTheme)
                    {
                        hoverItem = $("." + wijmoHoverClass)
                    }
                    else
                    {
                        hoverItem = $("." + notWijmoHoverClass)
                    }
                    if (key === 27)
                    {
                        self.close();
                    ...
                    else if (key === 38 || key === 40 || key === 9)
                    {
                        ...
                        if (self._useWijmoTheme)
                        {
                            hoverItem.addClass("filter-hover " + wijmoHoverClass)
                        }
                        else
                        {
                ...
                _GcFilterDialog.prototype._updateData = function(checkedValues)
                {
                    ...
                    if (!drf || !spread.features.conditionalFormat)
                    {
                        return
                    }
                    ...
                    for (var i = 0; i < checkedValues.length; i++)
                    {
                        ...
                        if (value === spread.SR.Blanks)
                        {
                            value = ""
                        }
                        var tc = new spread.TextCondition(0, value, keyword_null);
                ...
                _GcFilterDialog.prototype._getSortLink = function(id, text, callback)
                {
                    ...
                    }).text(text).addClass(filterSortClass).css({
                        display: "block", padding: "5px 6px", cursor: "default", margin: "2px 0px 1px 2px", "text-decoration": "none", outline: "none"
                    });
                    if (callback)
                    {
                        link.click = callback
                ...
                _GcFilterDialog.prototype._getItemsData = function(filter, col)
                {
                    ...
                    for (var row = range.row; row < range.row + range.rowCount; row++)
                    {
                        ...
                        if (text === "")
                        {
                            ...
                            {
                                hasBlank = true;
                                items.push(spread.SR.Blanks);
                                if (sheet.getRowHeight(row, 3) > 0)
                                {
                                    checkedItems.push(spread.SR.Blanks)
                                }
                            }
                            continue
                        }
                ...
                _GcFilterDialog.prototype._initFilterItemsLayout = function()
                {
                    ...
                    for (var i = 0, len = allValues.length; i < len; i++)
                    {
                        ...
                        itemOuterDiv.className = filterItemClass;
                        $(itemOuterDiv).css({
                            "padding-top": filterItemPadding / 2, "padding-bottom": filterItemPadding / 2, "border-width": self._itemDivBorder + "px", margin: self._itemDivMargin + "px", height: self._itemDivHeight + "px", "box-sizing": "content-box"
                        });
                ...
                _GcFilterDialog.prototype._refreshScrollbar = function()
                {
                    ...
                    if (isNeedVScroll)
                    {
                        ...
                        var scrollbarV = self._scrollbarV;
                        scrollbarV.useWijmoTheme(self._useWijmoTheme);
                    ...
                    if (isNeedHScroll)
                    {
                        ...
                        var scrollbarH = self._scrollbarH;
                        scrollbarH.useWijmoTheme(self._useWijmoTheme);
                ...
                {
                    var self = this;
                    self._scrollbarV = new spread.Scrollbar(false);
                ...
                {
                    var self = this;
                    self._scrollbarH = new spread.Scrollbar(true);
                ...
                _GcFilterDialog.prototype._setButtonState = function()
                {
                    ...
                    var okButton = $("#" + okID);
                    var useWijmoTheme = this._useWijmoTheme,
                        wijmoDiabledClassName = "ui-button-disabled ui-state-disabled",
                        notWijmoDiabledClassName = "filter-button-disable";
                    if (checkedLength > 0)
                    {
                        okButton.attr("disabled", false).removeClass(useWijmoTheme ? wijmoDiabledClassName : notWijmoDiabledClassName)
                    }
                    else
                    {
                        okButton.attr("disabled", true).addClass(useWijmoTheme ? wijmoDiabledClassName : notWijmoDiabledClassName)
                    }
                };
        ...
        spread._GcFilterDialog = _GcFilterDialog
    })(wijmo.spread || (wijmo.spread = {}));
    var spread = wijmo.spread
})(wijmo || (wijmo = {}));
var __extends = this.__extends || function(d, b)
...
(function(wijmo)
{
    (function(spread)
    {
        spread.feature("filter", ["core.common", "core.stringResource", "filter_ui"]);
        var keyword_null = null,
        ...
        var RowFilterBase = (function()
            {
                ...
                RowFilterBase.prototype.addFilterItem = function(col, filterItem)
                {
                    ...
                    {
                        throw new Error(spread.SR.Exp_FilterItemIsNull);
                    }
                    if (col < -1 || col >= self.sheet.getColumnCount())
                    {
                        throw new Error(spread.SR.Exp_InvalidColumnIndex);
                    }
                    if (!self.range)
                    {
                        return
                ...
                {
                    if (!spread.features.conditionalFormat)
                    {
                        return
                    }
                    var con = new spread.AverageCondition(compareType);
                ...
                RowFilterBase.prototype.addBackgroundFilter = function(col, color)
                {
                    ...
                    var con = new spread.ColorCondition(0, color);
                    this.addFilterItem(col, con)
                };
                RowFilterBase.prototype.addDateFilter = function(col, compareType, date)
                {
                    ...
                    var con = new spread.DateCondition(compareType, date);
                    this.addFilterItem(col, con)
                };
                RowFilterBase.prototype.addForegroundFilter = function(col, color)
                {
                    ...
                    var con = new spread.ColorCondition(1, color);
                    this.addFilterItem(col, con)
                };
                RowFilterBase.prototype.addNumberFilter = function(col, compareType, num)
                {
                    ...
                    var con = new spread.NumberCondition(compareType, num);
                    this.addFilterItem(col, con)
                };
                RowFilterBase.prototype.addTextFilter = function(col, compareType, text)
                {
                    ...
                    var con = new spread.TextCondition(compareType, text);
                    this.addFilterItem(col, con)
                };
                RowFilterBase.prototype.addTop10Filter = function(col, compareType, rank)
                {
                    ...
                    var con = new spread.Top10Condition(compareType, rank);
                    this.addFilterItem(col, con)
                };
                ...
                RowFilterBase.prototype._getItemType = function(col)
                {
                    ...
                    for (var i = 0; i < len; i++)
                    {
                        ...
                        if (filterItem.conditionType === "RelationCondition" || filterItem instanceof spread.RelationCondition)
                        {
                            if (filterItem.item1)
                            {
                                if (filterItem.item1.conditionType === "TextCondition" || filterItem.item1.conditionType === "TextLengthCondition")
                                {
                                    if (spread.ArrayHelper.indexOf(itemtypeArray, "T") === -1)
                                    {
                                ...
                                else if (filterItem.item1.conditionType === "ColorCondition")
                                {
                                    ...
                                    {
                                        if (spread.ArrayHelper.indexOf(itemtypeArray, "BC") === -1)
                                        {
                                    ...
                                    {
                                        if (spread.ArrayHelper.indexOf(itemtypeArray, "FC") === -1)
                                        {
                                ...
                                {
                                    if (spread.ArrayHelper.indexOf(itemtypeArray, "V") === -1)
                                    {
                ...
                {
                    return spread.ArrayHelper.contains(this.filteredColumns, col)
                };
                ...
                RowFilterBase.prototype._addFilteredInRowsWithColumnIndex = function(row, col)
                {
                    ...
                    {
                        var arrayHelper = spread.ArrayHelper;
                ...
                {
                    if (!spread.ArrayHelper.contains(this.filteredItems, filterItem))
                    {
                ...
                RowFilterBase.prototype._setColumnFilteredState = function(col, isFiltered)
                {
                    ...
                    if (isFiltered)
                    {
                        if (self.filteredColumns.length > 0)
                        {
                            ...
                            {
                                spread.ArrayHelper.remove(self.filteredColumns, col)
                            }
                        }
                ...
                RowFilterBase.prototype._removeFilteredItems = function(col)
                {
                    ...
                    for (var i = 0; i < removeItems.length; i++)
                    {
                        ...
                        if (!spread.ArrayHelper.contains(otherItems, item))
                        {
                            spread.ArrayHelper.remove(self.filteredItems, item)
                        }
                    }
                };
                RowFilterBase.prototype._removeFilteredInRowsWithColumnIndex = function(col)
                {
                    for (var key in this.filteredInRowsWithColIndexs)
                    {
                        if (key)
                        {
                            ...
                            if (colIndexs && spread.ArrayHelper.contains(colIndexs, col))
                            {
                                spread.ArrayHelper.remove(colIndexs, col);
                    ...
                    for (var i = 0; i < filterItems.length; i++)
                    {
                        ...
                        if (filterItem.conditionType === "RelationCondition" || filterItem instanceof spread.RelationCondition)
                        {
                            if (filterItem.item1)
                            {
                                ...
                                if (filterItem.item1.conditionType === "Top10Condition" || filterItem.item1 instanceof spread.Top10Condition)
                                {
                                    filterItem.item1.ranges = self._getReviseRanges(col, self.range)
                                }
                                else if (filterItem.item1.conditionType === "AverageCondition" || filterItem.item1 instanceof spread.AverageCondition)
                                {
                                    filterItem.item1.ranges = self._getReviseRanges(col, self.range)
                                }
                            }
                            if (filterItem.item2)
                            {
                                ...
                                if (filterItem.item2.conditionType === "Top10Condition" || filterItem.item2 instanceof spread.Top10Condition)
                                {
                                    filterItem.item2.ranges = self._getReviseRanges(col, self.range)
                                }
                                else if (filterItem.item2.conditionType === "AverageCondition" || filterItem.item2 instanceof spread.AverageCondition)
                                {
                                    filterItem.item2.ranges = self._getReviseRanges(col, self.range)
                                }
                            }
                        ...
                        else
                        {
                            ...
                            if (filterItem.conditionType === "Top10Condition" || filterItem instanceof spread.Top10Condition)
                            {
                                filterItem.ranges = self._getReviseRanges(col, self.range)
                            }
                            else if (filterItem.conditionType === "AverageCondition" || filterItem instanceof spread.AverageCondition)
                            {
                                filterItem.ranges = self._getReviseRanges(col, self.range)
                            }
                        }
                ...
                RowFilterBase.prototype._getReviseRanges = function(col, range)
                {
                    ...
                    if (range)
                    {
                        ...
                        {
                            reviseRange.push(new spread.Range(actualRange.row, col, actualRange.rowCount, 1))
                        }
                    }
                    return reviseRange
                };
                ...
                RowFilterBase.prototype._isRowfilteredOutByColumn = function(row, col)
                {
                    ...
                    if (self.filteredInRowsWithColIndexs)
                    {
                        ...
                        var currentIndex = spread.ArrayHelper.indexOf(self.filteredColumns, col);
                        if (currentIndex > 0)
                        {
                ...
                RowFilterBase.prototype._addRows = function(row, count)
                {
                    ...
                    if (self.range.row > -1)
                    {
                        ...
                        if (cs.row >= row)
                        {
                            self._setRangeInternal(new spread.Range(cs.row + count, cs.col, cs.rowCount, cs.colCount))
                        }
                        else if (cs.row < row && row < cs.row + cs.rowCount)
                        {
                            self._setRangeInternal(new spread.Range(cs.row, cs.col, cs.rowCount + count, cs.colCount))
                        }
                    }
                ...
                RowFilterBase.prototype._addColumns = function(column, count)
                {
                    ...
                    if (cs.col > -1)
                    {
                        if (cs.col >= column)
                        {
                            ...
                            self._setRangeInternal(new spread.Range(cs.row, cs.col + count, cs.rowCount, cs.colCount));
                        ...
                        else if (cs.col < column && column < cs.col + cs.colCount)
                        {
                            ...
                            self._setRangeInternal(new spread.Range(cs.row, cs.col, cs.rowCount, cs.colCount + count))
                        }
                    }
                    if (startColumn < 0)
                    {
                    ...
                    for (c = startColumn + columnCount - 1; c >= startColumn; c--)
                    {
                        if (c >= column)
                        {
                            ...
                            var index = spread.ArrayHelper.indexOf(self.filteredColumns, c);
                            if (index >= 0)
                            {
                            ...
                            {
                                spread.ArrayHelper.remove(self.filterItemMap, c);
                ...
                RowFilterBase.prototype._removeRows = function(row, count)
                {
                    ...
                    if (self.range.row > -1)
                    {
                        ...
                        if (cs.row >= row)
                        {
                            ...
                            {
                                self._setRangeInternal(new spread.Range(row, cs.col, (cs.row + cs.rowCount) - (row + count), cs.colCount))
                            }
                            else
                            {
                                self._setRangeInternal(new spread.Range(cs.row - count, cs.col, cs.rowCount, cs.colCount))
                        ...
                        {
                            self._setRangeInternal(new spread.Range(cs.row, cs.col, cs.rowCount - Math.min(cs.row + cs.rowCount - row, count), cs.colCount))
                        }
                    }
                    var filteredInRowsWithColIndexs = self.filteredInRowsWithColIndexs;
                    if (self.isFiltered() && filteredInRowsWithColIndexs)
                ...
                RowFilterBase.prototype._removeColumns = function(column, count)
                {
                    ...
                    if (cs.col >= 0)
                    {
                        if (column < cs.col)
                        {
                            ...
                            {
                                self._setRangeInternal(new spread.Range(cs.row, column, cs.rowCount, (cs.col + cs.colCount) - removeEnd))
                            }
                            else
                            {
                                self._setRangeInternal(keyword_null)
                        ...
                        else if (column < cs.col + cs.colCount)
                        {
                            ...
                            {
                                self._setRangeInternal(new spread.Range(cs.row, cs.col, cs.rowCount, column - cs.col))
                            }
                        }
                    }
                    self.reFilter()
                ...
                RowFilterBase.prototype._clear = function(row, column, rowCount, columnCount)
                {
                    ...
                    var clearRange = new spread.Range(row, column, rowCount, columnCount);
                    if (self.filterButtonVisible())
                    {
                        ...
                        if (clearRange.containsRange(new spread.Range(newRow, self.range.col, newRowCount, self.range.colCount)))
                        {
                            self.unfilter()
                        }
                    }
                ...
                RowFilterBase.prototype._getConditionTypeDictionary = function()
                {
                    ...
                    {
                        var dict = {};
                        dict[0] = spread.RelationCondition;
                        dict[1] = spread.NumberCondition;
                        dict[2] = spread.TextCondition;
                        dict[3] = spread.ColorCondition;
                        dict[4] = spread.FormulaCondition;
                        dict[5] = spread.DateCondition;
                        dict[6] = spread.DateExCondition;
                        dict[7] = spread.TextLengthCondition;
                        dict[8] = spread.Top10Condition;
                        dict[9] = spread.UniqueCondition;
                        dict[10] = spread.AverageCondition;
                        dict[11] = spread.CellValueCondition;
                        dict[12] = spread.AreaCondition;
                ...
                RowFilterBase.prototype.fromJSON = function(settings, isNoneSchema)
                {
                    ...
                    if (jsonRange)
                    {
                        self.range = new spread.Range(jsonRange.row, jsonRange.col, jsonRange.rowCount, jsonRange.colCount)
                    }
                    var jsonFilterItemMap = settings.filterItemMap,
                        selfFilterItemMap = self.filterItemMap;
                    if (spread.features.conditionalFormat && jsonFilterItemMap)
                    {
        ...
        spread.RowFilterBase = RowFilterBase;
        var HideRowFilter = (function(_super)
            {
                ...
                {
                    var filterDialog = new spread._GcFilterDialog(this.sheet, filterButtonHitInfo);
                    this.sheet._filterDialiog = filterDialog;
                    filterDialog.open()
                };
                return HideRowFilter
            })(RowFilterBase);
        spread.HideRowFilter = HideRowFilter;
        ...
        spread.FilterRowsVisibleInfo = FilterRowsVisibleInfo;
        ...
        spread._TableFilter = _TableFilter
    })(wijmo.spread || (wijmo.spread = {}));
    var spread = wijmo.spread
})(wijmo || (wijmo = {}))
