...
(function(wijmo)
{
    (function(spread)
    {
        (function(Calc)
        {
            (function(Functions)
            {
                spread.feature("calc.functions_db", ["calc.functions"]);
                ...
                var _DatabaseEnumerator = (function()
                    {
                        function _DatabaseEnumerator(database, field, criteria)
                        {
                            ...
                            {
                                throw spread.SR.Exp_InvalidNull;
                            }
                        }
                        ...
                        _DatabaseEnumerator.prototype.current = function()
                        {
                            ...
                            {
                                throw spread.SR.Exp_InvalidOperation;
                            }
                        ...
                        _DatabaseEnumerator.prototype.createEvaluator = function(criteria)
                        {
                            if (typeof criteria === "string")
                            {
                                ...
                                var textHelper = new spread._StringHelper(criteriaText);
                                ...
                                for (var i = 0; i < _DatabaseEnumerator.operators.length; i++)
                                {
                                    if (textHelper.startsWith(_DatabaseEnumerator.operators[i].getName()))
                                    {
                                        ...
                                        if (!isNaN(criteriaDouble = spread._NumberHelper.parseInvariant(criteriaText)))
                                        {
                            ...
                            for (var i = 1; !pass && i < self.criteria.rowCount; i++)
                            {
                                ...
                                for (var j = 0; pass && j < self.criteria.colCount; j++)
                                {
                                    ...
                                    if (criteriaEva)
                                    {
                                        ...
                                        if ((criteriaEva.criteriaOperator.name === "=") && (typeof criteriaEva.criteriaValue === "string") && (typeof databaseValue === "string") && (wildcardCriteria = spread.RegUtil.getWildcardCriteriaFullMatch(criteriaEva.criteriaValue)))
                                        {
                                            pass = spread.RegUtil.getRegIgnoreCase(wildcardCriteria).test(databaseValue)
                                        }
                                        else
                                        {
                ...
                function db_daverage(args)
                {
                    ...
                    {
                        throw spread.SR.Exp_ArgumentNull;
                    }
        ...
        })(spread.Calc || (spread.Calc = {}));
        var Calc = spread.Calc
...
(function(wijmo)
{
    (function(spread)
    {
        (function(Calc)
        {
            (function(Functions)
            {
                spread.feature("calc.functions_eng", ["calc.functions"]);
                var keyword_null = null,
                    CalcConvert = Calc.Convert,
                    Math_abs = Math.abs,
                    Math_sin = Math.sin,
...
(function(wijmo)
{
    (function(spread)
    {
        (function(Calc)
        {
            (function(Functions)
            {
                spread.feature("calc.functions_fin", ["calc.functions"]);
                var keyword_null = null,
                    CalcConvert = Calc.Convert,
                    Math_pow = Math.pow,
                    Math_abs = Math.abs,
...
(function(wijmo)
{
    (function(spread)
    {
        (function(Calc)
        {
            (function(Functions)
            {
                spread.feature("calc.functions_lookup", ["calc.functions"]);
                var $ = jQuery,
                    keyword_null = null,
                ...
                function value_compare(a, b, case_sensitive)
                {
                    ...
                    if (typeof a === "string")
                    {
                        ...
                        {
                            date = wijmo.spread._DateTimeHelper._parseDate(a);
                            if (!isNaN(date))
                            {
                                if (date === b)
                                {
                ...
                function find_index_linear(find, data, type, height)
                {
                    ...
                    if (isFindIsString)
                    {
                        var wildcardCriteria = spread.RegUtil.getWildcardCriteriaFullMatch(realVal);
                        if (wildcardCriteria)
                        {
                            regExp = spread.RegUtil.getRegIgnoreCase(wildcardCriteria)
                        }
                    }
                ...
                var _CellInfoReference = (function(_super)
                    {
                        ...
                        {
                            _super.call(this, source, [new wijmo.spread.Range(row, column, rowCount, columnCount)]);
                            var self = this;
                            self._row = row;
                            self._column = column;
                ...
                function eg_address(args)
                {
                    ...
                    var sb = new spread._StringBuilder;
                    ...
                    if (args[0] instanceof Calc.Reference)
                    {
                        ...
                        if (args.length == 2)
                        {
                            ...
                            if (arrayRowCount * arrayColumnCount < index)
                                return CalcErrorsReference;
                            if (index == 0)
                                return reference.create([new spread.Range(arrayRow, arrayCol, arrayRowCount, arrayColumnCount)]);
                            else
                            {
                                var offsetRow = Math.floor((index - 1) / arrayColumnCount) + arrayRow;
                                var offsetCol = (index - 1) % arrayColumnCount + arrayCol;
                                return reference.create([new spread.Range(offsetRow, offsetCol, 1, 1)])
                            }
                        }
                        else
                        {
                            ...
                            if (arrayRowCount <= row || arrayColumnCount <= column || reference.getRangeCount() <= area)
                                return CalcErrorsReference;
                            if (row == -1 && column == -1)
                                return reference.create([new spread.Range(sourceRow, sourceCol, arrayRowCount, arrayColumnCount)]);
                            else if (row == -1)
                                return reference.create([new spread.Range(sourceRow, offsetCol, arrayRowCount, 1)]);
                            else if (column == -1)
                                return reference.create([new spread.Range(offsetRow, sourceCol, 1, arrayColumnCount)]);
                            else
                                return reference.create([new spread.Range(offsetRow, offsetCol, 1, 1)])
                        }
                    }
                ...
                function eg_indirect(args, context)
                {
                    ...
                    var parserContext = new Calc.ParserContext(context.source, !useA1, baseRow, baseCol);
...
(function(wijmo)
{
    (function(spread)
    {
        (function(Calc)
        {
            (function(Functions)
            {
                spread.feature("calc.functions_stat", ["calc.functions"]);
                var keyword_null = null,
                    Math_pow = Math.pow,
                    Math_max = Math.max,
                    Math_min = Math.min,
